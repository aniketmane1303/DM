...
const ClusterButtons = ({ clusterType, onButtonClick, selectedCluster }) => (
  <button
    onClick={() => onButtonClick(clusterType)}
    className={`cluster-button ${selectedCluster === clusterType ? 'selected' : ''}`}
  >
    {clusterType}
  </button>
);
...
const Assignment6 = () => {
  ...
  useEffect(() => {
    const fetchImagePaths = async () => {
      ...
      const updatedImagePaths = paths.reduce(
        (acc, { clusterType, imagePath }) => {
          acc[clusterType] = imagePath;
          return acc;
        },
  ...
  return (
    <div className="assignment6-container">
      <div className="header">
        ...
        <p>Select clustering algorithms and tabulate results</p>
      </div>

      <div className="cluster-buttons-container">
        {['agnes', 'kmeans', 'pam', 'birch', 'dbscan'].map((clusterType) => (
          <ClusterButtons
            key={clusterType}
            clusterType={clusterType}
        ...
        <button onClick={handleTabulateResults} className="tabulate-button">
          Tabulate Results
        </button>
      </div>
      ...
      <div className="images-container">
        {selectedClusters.map((clusterType) => (
          <div key={clusterType} className="image-wrapper">
            <p className="image-label">{`Image after ${clusterType} clustering:`}</p>
            <img
              src={`http://127.0.0.1:8000${imagePaths[clusterType]}`}
              alt={`${clusterType} Clustering`}
