...
const NavbarItem = ({ title, to, selected, setSelected }) => {
  ...
  return (
    <IconButton
      ...
      color={selected === title ? "primary" : "default"}
      sx={{
        color: selected === title ? theme.palette.primary.main : "inherit",
        "&:hover": {
...
const Navbar = () => {
  ...
  return (
    <AppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1, backgroundColor: theme.palette.secondary.main }}>
      <Toolbar>
        <IconButton edge="start" color="inherit" aria-label="menu">
          <MenuOutlinedIcon />
        </IconButton>
        ...
        <NavbarItem title="Dashboard" to="/" selected={selected} setSelected={setSelected} />
        {/* ... (remaining NavbarItems) */}
        <NavbarItem
          title="Assignment 1"
          to="/assignment1"
          selected={selected}
